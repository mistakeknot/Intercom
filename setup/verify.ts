...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.intercom')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.intercom'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active intercom`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8' });
        if (output.includes('intercom')) {
          service = 'stopped';
        }
      } catch {
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'intercom', 'mount-allowlist.json'))) {
